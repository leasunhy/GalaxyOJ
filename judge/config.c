...
const struct lang_t lang[] = {
	{"C", "gcc", "c", "gcc -o %s %s -O -ansi -Wall -lm --static > %s.log 2>&1"},
	{"C++", "g++", "cpp", "g++ -o %s %s -O -ansi -Wall -lm --static > %s.log 2>&1"},
	{"Pascal", "fpc", "pas", "fpc -o%s %s -O > %s.log 2>&1"},
...
static const struct conf_strings_t {
	...
	{"source-dir", SRC_PREFIX},
	{NULL, NULL},
};

static const char *sopts = "H:u:p:d:D:s:j:c:h";
